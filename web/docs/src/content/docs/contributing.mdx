...
The documentation website sources are Markdown
files located in the [`web/docs/src/content/docs/`
directory](https://github.com/jj-vcs/jj/tree/main/web/docs/src/content/docs).
You do not need to know Rust to work with them. See below for [instructions on
how to preview the HTML docs](#previewing-the-html-documentation) as you edit
the Markdown files. Doing so is optional, but recommended.
...
One-time setup:
    ...
    rustup toolchain add 1.97.1   # also specified in Cargo.toml
...
3. Your code will be rejected if it cannot be compiled with the minimal
   ...
   As of this writing, that version is **1.97.1**.
...
<details>
<summary> Some details and speculation </summary>
...
[https://docs.jj-vcs.dev/](https://docs.jj-vcs.dev/).
...
The only thing you need is `npm`, a JavaScript package manager (built on the
`node` runtime).
...
<Tabs>
  ...
  <TabItem label="Other options">
    See the [instructions on nodejs.org](https://nodejs.org/en/download) for
...
To build the docs, run while inside the `web/docs/` directory:
...
Open [http://localhost:4321](http://localhost:4321) in your browser to see the
docs.

As you edit the `.md` files in `web/docs/src/content/docs/`, the website should
be rebuilt and reloaded in your browser automatically.

:::note[If the docs are not updating]
Try restarting the `npm run dev` process and reloading the browser tab.
:::

#### Offline distribution

To build the rendered docs for offline distribution or to view them from your
file system, run while inside the `web/docs/` directory:
...
documentation that we currently do with a script run on every release.
:::
...
URL [https://docs.jj-vcs.dev](https://docs.jj-vcs.dev) redirects to
[https://docs.jj-vcs.dev/latest](https://docs.jj-vcs.dev/latest), which is a
copy of the docs for the last stable version.

The different versions of documentation are managed and
deployed with the script in [`.github/scripts/docs-build-deploy`
](https://github.com/jj-vcs/jj/blob/main/.github/scripts/docs-build-deploy).
...
1. Make sure `github.com/jjfan/jj` includes the `gh-pages` bookmark of the jj
   repo and run `git fetch origin gh-pages`.

1. Go to the GitHub repository settings, enable GitHub Pages, and configure them
   to use the `gh-pages` bookmark (this is usually the default).

1. For your fork, the website will be served at `https://jjfan.github.io/jj/`.
   The `/jj/` prefix in the path isn't present on the main website, so you need
   to configure this different "site base" in a few locations.

   - In the script `.github/scripts/docs-build-deploy`, change the value of
     `--base` to `"/jj/$site/"`.
   - In `web/docs/astro.config.mjs`, change the value of `site` from
     `https://docs.jj-vcs.dev` to `https://jjfan.github.io`.
   - In `web/docs/src/components/VersionSwitcher.astro`, change the value of
     `const siteBase = '/'` to `'/jj/'`.

   Omit these changes if you're submitting a PR to the upstream jj repository.

1. Install `npm` as explained in [Previewing the HTML
   documentation](#previewing-the-html-documentation), and run the same `sh`
...
1. Now, you should be able to see the full website, including
   your latest changes to the `prerelease` version, at
   `https://jjfan.github.io/jj/prerelease/`.

1. (Optional) The previous steps actually only rebuild
   `https://jjfan.github.io/jj/prerelease/`. If you'd like to test out version
   switching back and forth, you can also rebuild the docs for the latest
   release as follows.

   ```shell
   jj new v1.33.1  # Let's say `jj 1.33.1` is the latest release
   .github/scripts/docs-build-deploy v1.33.1
   ```

1. (Optional) When you are done, you may want to reset the `gh-pages` bookmark
   to the same spot as it is upstream. If you configured the "upstream" remote,
...
The script builds the site for the specified version. If the version is
"prerelease", then that's the only change. If the version is something else,
the script also updates the "latest" alias and records the new version in the
`versions.json` file, which is used by the version switcher. These changes are
then committed and pushed to the `gh-pages` bookmark.

If you need to do something more complicated, you can modify the script
to your liking or edit the `gh-pages` bookmark directly, but take care
to avoid files that will be overwritten by future invocations of the
script. Then, you can submit a PR based on the `gh-pages` bookmark of
[https://docs.jj-vcs.dev/](https://docs.jj-vcs.dev/) (instead of the usual
`main` bookmark).
